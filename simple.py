...
def sim(g_sim,fN,T,dataFolder):
    ...
    rate_dict={'init_state':g_sim.state,'rate':g_sim.rate,'T':T}
    t=0.
    while t<T:
...
def al_simple(state,rate,input_var):
    al={(('C',+1),):np.sum(rate['alpha_C_poly'][0]*np.array(state['E'])**rate['alpha_C_poly'][1]),
        (('C',-1),):np.sum(rate['mu_C_poly'][0]*(state['C']*(np.array(state['E'])**rate['mu_C_poly'][1]))),
        (('E',+1),):np.sum(rate['alpha_E_poly'][0]*np.array(state['C'])**rate['alpha_E_poly'][1]),
        (('E',-1),):rate['mu_E']*state['E']}
    return al
...
def init_simple_default(param={'state':{},'rate':{}},architecture='deg'):
    ...
    rate={'alpha_C_poly':((5,),(0,)),
          'mu_C_poly':((0.3,30),(0,1)),
          'mu_E':0.1,
          'alpha_E_poly':((1,),(1,)),
          'architecture':architecture
          }
    if architecture=='noDeg':
        rate['mu_C_poly']=((0.3,),(0,))
    elif architecture=='noInfo':
        rate['alpha_E_poly']=((0.04,),(0,))
...
def continuous_bound_plot(picFN,newDic_list,newRate_list,prevPicDicFN='simple',x1var='C',x2var='E'):
    ...
    key_ordering=('architecture','alpha_E_poly','alpha_C_poly','mu_C_poly','mu_E')
...
def get_dic(fN,x1var,x2var,x2_birth_event_keys):
    ...
    tau_func=lambda x2,rate:1/np.sum(rate['mu_C_poly'][0]*np.array([ np.sum(x2**n*t_weight)/dur for n in rate['mu_C_poly'][1] ]))
    tau=tau_func(x2x2,rate)
    
#    bb=pickle.load(open(dataFolder+fN+'_event.p','rb'))
#    x2_birth=np.sum([bb[x] for x in x2_birth_event_keys])
########## x2_birth need to be divided by 2 if directly taken from events...!!!!!!!!!!!!!! #############
    
    x2_birth=np.sum(np.diff(x2x2)==1)
    f_mean2=x2_birth/dur
    f_mean=x1_mean*rate['alpha_E_poly'][0][0]
    print(f_mean2-f_mean)
    n2=f_mean*tau
    
    f_var=x1_var*rate['alpha_E_poly'][0][0]**2
...
#    pdb.set_trace()        
    ...
    lam=rate['alpha_C_poly'][0][0]
    dic=dict([('T',temp['T']),('lambda',lam),('tau',tau),('f_mean',f_mean),('f_var',f_var),('x1_var',x1_var),('x1_mean',x1_mean),('x2_mean',x2_mean),('x2_birth',x2_birth),('n2',n2),('architecture',rate['architecture']),('n2/n1',n2/x1_mean),('fano',x1_var/x1_mean)])
    return dic,rate
...
#    param_random_gen(N)
    architecture='deg'
    rate={'alpha_C_poly':((10,),(0,)),
          'mu_C_poly':((10000,-2000,100,),(0,1,2,)),
          'mu_E':1,
          'alpha_E_poly':((10,),(1,))
          }
    if architecture=='noDeg':
...
#        
    picFN=prefix+'bound'
    continuous_bound_plot(picFN,newDic_list,newRate_list,prevPicDicFN='simple_ctsPlot_bound')
